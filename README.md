...
- **Microanatomy** (`site/microanatomy.html`) — the brain at the cell's scale. Original drawings generated from code: a large multipolar neuron (`scripts/cells/multipolar-neuron/`, needs numpy and shapely) and a layer V cortical pyramidal cell standing in its layers (`scripts/cells/pyramidal-neuron/`, standard library only), with every structure a click target, an index that describes each, a colour-everything view and a test-yourself mode that asks for each from memory. The tiles above the plate switch cells without a reload. More cells follow the same two-file pattern in `site/assets/cells/`: one SVG marked with `data-part` and `data-vis`, one JSON naming the structures and the frame to draw them in.
