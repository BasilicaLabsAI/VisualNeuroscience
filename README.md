...
- **Microanatomy** (`site/microanatomy.html`) — the brain at the cell's scale. An original drawing of a large multipolar neuron generated from code (`scripts/cells/multipolar-neuron/`), with seventeen clickable structures, an index that describes each, a colour-everything view and a test-yourself mode that asks for each structure from memory. More cells follow the same two-file pattern in `site/assets/cells/`.
